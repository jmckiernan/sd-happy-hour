...
- **SMS opt-in** lives in My Stuff → **Preferences** (`/account/#section-preferences`): add a phone number, check the opt-in box, and use **Send test text** to confirm Twilio is wired up. Then enable the **Text** channel on any alert. The dispatch engine already skips text for users without phone + consent (falls through to email-only).
