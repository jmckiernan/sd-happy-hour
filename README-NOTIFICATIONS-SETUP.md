...
- **Restaurant accounts** at `/restaurant/` — a separate login from consumer accounts. Sign up with a business email; if it matches your website's domain, you're verified instantly. Otherwise your account goes into a review queue at `/admin/restaurants/` (sign in at `/account/` with an admin email — same admin list as everything else, see `README-ACCOUNTS-SETUP.md`).
- **Claiming a listing** — once verified, search for and link your existing venue listing from the dashboard. This is trusted, not admin-arbitrated (see "Known limitations" below).
- **Going live** — once linked, tap "We're live now" to mark your listing live immediately (auto-expires after 4 hours so a forgotten toggle doesn't stay on forever). This is on top of the normal weekly schedule already in `public/data/happy-hours.json` — either one showing "live" is enough to trigger alerts and the site's Live Now badge.
...
- **Venue linking is self-service, not admin-reviewed.** Only a restaurant's *identity* (the domain-match / manual-claim verification) goes through review at `/admin/restaurants/` — once verified, a restaurant can link itself to any existing venue listing by searching for it, with no separate approval of that specific link. Fine for a small/trusted early restaurant base; if listings start getting claimed by the wrong account, add an admin approval step to `PUT /api/restaurant/venue` (`src/pages/api/restaurant/venue.ts`).
- **New listings still go through `/submit/`.** A restaurant with no existing listing needs one created and approved the normal way (see `README-ACCOUNTS-SETUP.md`) before they have anything to claim/link. Wiring submission approval to automatically link back to the submitting restaurant account is a reasonable follow-up, not built here.
