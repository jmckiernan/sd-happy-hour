...
import { cancelPromotion } from '../../../../../lib/promotionService';
...
export const POST: APIRoute = async ({ params, request, cookies }) => {
  ...
  try {
    ...
    if (!body || Array.isArray(body) || Object.keys(body).length) {
      return errorJson(['Cancel does not accept request fields.'], 400);
    }
  } catch {
    return errorJson(['Invalid JSON body.'], 400);
  }
  try {
    const result = await cancelPromotion(session.userId, params.id || '');
    await captureMerchantEvent({
      eventName: 'campaign_pause', venueId: result.promotion.venueId,
