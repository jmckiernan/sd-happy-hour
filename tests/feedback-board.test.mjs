...
async function testSchemaAndSurfaces() {
  const [migration, layout, featuresPage, bugPage, featureRoute, voteRoute, bugRoute, store] = await Promise.all([
...
console.log('feedback board: matching, auth gates, one-vote schema, stored bug reports, and UI contracts passed');
