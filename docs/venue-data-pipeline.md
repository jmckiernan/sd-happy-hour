...
It only covers brands running a recognized locator widget. The complete fix is to route discovery through the AI deep extract, which reads any site the way a refresh does — **not enabled**, because it is roughly `$0.0085 × ~4,700 enriched candidates ≈ $40 per full import` against ~$7–14 for a refresh pass. Turn it on by having `resolveHappyHour` call `extractWebsiteHappyHourWithAi` instead of `extractWebsiteHappyHour`, and expect the discovery bill, not the refresh bill.
