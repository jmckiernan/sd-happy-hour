 ...
 *
 * Crawls each sampled venue's website with the importer's deep inventory and freezes
 * page text so extraction and hand review share the same bytes. Uses createCachedFetch
 * so warm page cache is reused; Playwright only when the cache misses and the site needs it.
 */
  ...
  await mapPool(venues, 4, async (venue) => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        ...
        media,
        social,
        blocked: Boolean(inventory.blocked),
    ...
    console.log(
      `  ${String(venue.id).padStart(5)} ${venue.name.slice(0, 32).padEnd(32)} ${pages.length} page(s), ${media.length} media, ${social.length} social${inventory.blocked ? ', BLOCKED' : ''}`
    );
  });
